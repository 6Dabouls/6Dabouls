...
function handleLogin() {
  ...
  if (!email || !email.includes('@')) {
    setAuthError('f-login-email', 'err-login-email', 'Email invalide.');
    valid = false;
  }
  if (!pwd || pwd.length < 1) {
    setAuthError('f-login-password', 'err-login-pwd', 'Mot de passe requis.');
    valid = false;
  }
  if (!valid) return;
...
function handleRegister() {
  ...
  const nameEl     = document.getElementById('f-reg-name');
  const emailEl    = document.getElementById('f-reg-email');
  const phoneEl    = document.getElementById('f-reg-phone');
  const pwdEl      = document.getElementById('f-reg-password');
  const confEl     = document.getElementById('f-reg-confirm');
  const codeEl     = document.getElementById('f-reg-code');
  ...
  if (!name || name.length < 2) {
    setAuthError('f-reg-name', 'err-reg-name', 'Prénom et nom requis (min. 2 caractères).');
    valid = false;
  }
  if (!email || !email.includes('@')) {
    setAuthError('f-reg-email', 'err-reg-email', 'Email invalide.');
    valid = false;
  }
  if (!phone || phone.replace(/\D/g, '').length < 8) {
    setAuthError('f-reg-phone', 'err-reg-phone', 'Numéro de téléphone invalide (8-9 chiffres).');
    valid = false;
  }
  if (!password || password.length < 6) {
    setAuthError('f-reg-password', 'err-reg-pwd', 'Minimum 6 caractères.');
    valid = false;
  }
  if (password !== confirm) {
    setAuthError('f-reg-confirm', 'err-reg-confirm', 'Les mots de passe ne correspondent pas.');
    valid = false;
  }
  if (!inviteCode || inviteCode.length < 5) {
    setAuthError('f-reg-code', 'err-reg-code', "Code d'invitation requis.");
    valid = false;
  }
  if (!valid) return;
  ...
  setTimeout(() => {
    const result = DB.register({ name, email, password, inviteCode, phone });
